...
func AdminEndpoint(c *fiber.Ctx) error {
	...
	if !IsAdmin {
		if login == credentials.AdminLogin && password == credentials.AdminPassword {
			IsAdmin = true
			fmt.Println("admin", credentials.AdminLogin, "successfully logged in")
			data_admin := struct {
				IsLoggedIn bool
			}{
				IsLoggedIn: IsAdmin,
			...
			fmt.Println("admin", credentials.AdminLogin, "wrong login or password")
			return c.SendString("wrong login or password")
		}
	} else {
		AddArticle(c)
		fetchPosts()
...
func AddArticle(c *fiber.Ctx) {
	p := Post{
		Author:  "Admin",
		Title:   "Randon title",
		Content: NewMarkdown(strings.NewReader(c.FormValue("article"))),
	}
	addNewPost(p)
}
